...
def filtering(imgFilePath, xmlFilePath):
    xmlRoot = ET.parse(xmlFilePath).getroot()

    # 画像読み込み
    im = cv2.imread(imgFilePath)
   
    for idx, obj in enumerate(xmlRoot.findall('.//object')):
        ...
        cutedimg = cut_img.shape[:2][::-1]
        print (idx, imgFilePath, cut_img.shape)
        if (cut_img.shape[0] < 10 or cut_img.shape[1] < 10):
            size = min((cut_img.shape[0], cut_img.shape[1], cutedimg[0]))
            print (idx, cutedimg[0], cutedimg[1], size)
            cut_img = cv2.resize(cut_img, (cutedimg[0]/4, cutedimg[0]/4))
        elif (cut_img.shape[0] < 20 or cut_img.shape[1] < 20):
            cut_img = cv2.resize(cut_img, (cutedimg[0]/10, cutedimg[0]/10))
        else: 
            cut_img = cv2.resize(cut_img, (cutedimg[0]/20, cutedimg[0]/20))
        cut_img = cv2.resize(cut_img, cutedimg, interpolation = cv2.cv.CV_INTER_NN)
...
if __name__ == "__main__":
    ...
    for imgfname in files:
        ...
        result = filtering(imgfname, xmlfname)
